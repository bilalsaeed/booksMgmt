...
namespace booksmanagement.Controllers
{
    ...
    public class CarPartComponentsController : Controller
    {
                ...
                if (Request.Files.Count > 0)
                {
                    ...
                    for (int i = 0; i < files.Count; i++)
                    {
                        ...
                        if (file != null && file.ContentLength > 0)
                        {
                            ...
                            drawingFile.CarPartComponentId = carPartComponent.Id;
                            db.DrawingFiles.Add(drawingFile);

                            carPartComponent.IsDrawingAvailable = true;
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<ActionResult> Edit([Bind(Include = "Id,Name,CarPartId,CreatedAt,IsDrawingAvailable")] CarPartComponent carPartComponent)
        {
            if (ModelState.IsValid)
            {
                if (Request.Files.Count > 0)
                {
                    ...
                    var oldDrawings = db.DrawingFiles.Where(d => d.CarPartComponentId == carPartComponent.Id).ToList();
                    var files = Request.Files;
                    for (int i = 0; i < files.Count; i++)
                    {
                        var file = files[i];
